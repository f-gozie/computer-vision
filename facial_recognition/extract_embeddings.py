...
for (i, imagePath) in enumerate(imagePaths):
	...
	imageBlob = cv2.dnn.blobFromImage(cv2.resize(image, (300, 300)), 1.0, (300, 300), (104.0, 
		177.0, 123.0), swapRB=False, crop=False)

	# apply OpenCV's deep learning-based face detector to localize faces in the output image
	detector.setInput(imageBlob)
	detections = detector.forward()

	# ensure at least one face was found
	if len(detections) > 0:
		# we're making the assumption that each image has only one face, so we are finding the 
		...
		# test (thus helping us find weaker detections)
		if confidence > args["confidence"]:
			...
			# construct a blob for the face ROI, then pass the blob through our face embedding
			# model to obtain the 128-d quantification of the face
